...
const appId = 'amzn1.ask.skill.aa6b3025-e2f3-4bb0-839b-87affc425319';
const recipesTable = 'RecipeScan';
...
const instructions = `Welcome to Meal Prep with recipes from BBC recipes.<break strength="medium" /> 
                      The following commands are available:
                      get recipe, get a random recipe, and delete recipe. What 
...
const handlers = {
  ...
  'GetRecipeIntent'() {
    ...
    dynamoParams.FilterExpression += ' and contains(#title, :recipeName)';
    ...
    //dbGet(dynamoParams).then(data => {
        ...
        //     item
        // });
        const recipe = data.Items[Math.floor(Math.random()*items.length)];
        //const recipe = data.Item;

        if (recipe) {
          this.emit(':tell', `Recipe is ${recipeName}`);
  ...
  'DeleteRecipeIntent'() {
    ...
    //dbGet(dynamoParams).then(data => {
        ...
        if (recipe) {
          ...
          docClient.delete(dynamoParams).promise().then(data =>{
